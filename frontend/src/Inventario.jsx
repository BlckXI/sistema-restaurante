...
export default function Inventario() {
  ...
  // --- ESTADOS DE FILTRO Y BÚSQUEDA (NUEVOS) ---
  ...
  const platosFiltrados = platos.filter(plato => {
    ...
    // 2. Filtro por categoría (si hay una seleccionada)
  ...
  const guardarPlato = async (e) => {
    e.preventDefault();
    if (!nombre.trim() || !precio || !stock || !categoria) {
        mostrarNotificacion("Completa todos los campos", "error");
        return;
    }
    setCargando(true);
    try {
      const datos = { nombre, precio: parseFloat(precio), stock: parseInt(stock), categoria };
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 h-[calc(100vh-100px)] relative p-2">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
                ...
                <div className="bg-gray-50 px-6 py-4 flex gap-3 justify-center">
                    <button onClick={cancelarEliminar} className="bg-white text-gray-700 border border-gray-300 font-bold py-2 px-6 rounded-lg hover:bg-gray-100 transition">Cancelar</button>
                    <button onClick={confirmarEliminar} className="bg-red-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-red-700 shadow-md transition">Sí, Eliminar</button>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="w-full md:w-1/3 space-y-6 overflow-y-auto">
        {/* FORM PLATO */}
        <div className={`p-6 rounded-lg shadow-md border transition-colors h-fit
            ${idEdicion ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200'}`}>
            ...
            <form onSubmit={guardarPlato} className="space-y-3">
                ...
                <div>
                    ...
                    <select className="w-full p-2 border rounded bg-white" value={categoria} onChange={e => setCategoria(e.target.value)}>
      ...
      {/* DERECHA: LISTA PLATOS CON BUSCADOR */}
        ...
        <div className="p-4 bg-gray-50 border-b space-y-3">
            <div className="flex justify-between items-center">
                ...
                <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full font-bold">
                    Viendo {platosFiltrados.length} de {platos.length}
                </span>
            </div>
            ...
            <div className="flex gap-2">
                ...
                />
                <select 
                    className="p-2 border rounded text-sm bg-white focus:ring-2 focus:ring-blue-200 outline-none"
                        ...
                        <tr key={plato.id} className={`hover:bg-blue-50 transition-colors ${idEdicion === plato.id ? 'bg-yellow-50' : ''}`}>
                            ...
                            <td className="p-3 text-right">
                                <div className="flex justify-end gap-2">
                                    <button 
                                        onClick={() => prepararEdicion(plato)} 
                                        className="text-blue-600 hover:bg-blue-100 font-bold text-sm px-3 py-2 rounded transition flex items-center gap-1"
                                    >
                                        ✏️
                                    </button>
                                    <button 
                                        onClick={() => solicitarEliminar(plato.id, 'plato', plato.nombre)}
                                        className="text-red-600 hover:bg-red-100 font-bold text-sm px-3 py-2 rounded transition flex items-center gap-1"
                                    >
                                        🗑️
                                    </button>
                                </div>
