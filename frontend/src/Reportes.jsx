...
export default function Reportes() {
    ...
    // ... (Mantén tu función generarPDF y las acciones de API igual, no causan pantalla blanca en render) ...
    // Para abreviar, asumo que generarPDF, registrarGasto, etc. siguen igual que tu código original.

    // --- GENERACIÓN DE PDF (Resumido para no repetir todo, usa tu función original) ---
    const generarPDF = () => {
         // Asegúrate de usar datos?.listaOrdenes || [] dentro de esta función también
         if (!datos) { mostrarNotificacion("No hay datos", "error"); return; }
         // ... tu lógica de PDF ...
         // Solo recuerda: donde hagas .forEach o .map, pon un ?. antes o || []
         // Ejemplo dentro del PDF: datos.listaOrdenes?.forEach(...)
         mostrarNotificacion("Función PDF omitida en esta vista previa para ahorrar espacio", "info");
    };
    
    // --- ACCIONES DE API (Las mismas de tu código original) ---
