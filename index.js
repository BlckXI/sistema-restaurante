...
app.get('/', (req, res) => { res.send('Servidor Restaurante V4.2 (Transferencias & Personal Fixed) 🚀'); });
...
// CREAR ORDEN (Incluye metodo_pago)
...
const { data, error } = await supabase.from('ordenes')
    .select('*')
    .eq('estado', 'pendiente')
    .order('numero_diario', { ascending: true }) 
